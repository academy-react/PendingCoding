...
export const CommentRespond = ({ commentId }) => {
  const { id } = useParams();
  const {
    data: responds,
    ...
    queryKey: ["news_comment_responds", commentId],
  ...
  const [likeCount, setLikeCount] = useState(responds?.commentLike);
  // const [disLikeCount, setDisLikeCount] = useState(responds?.disslikeCount);
  ...
  const differenceInDays = getPersianNumbers(
    Math.round(
      (new Date().getTime() - new Date(responds?.insertDate).getTime()) /
        (1000 * 3600 * 24)
    )
  );
  const postDate = new Date(responds?.insertDate)
  ...
  const onSubmit = async (values) => {
    try {
      setIsLoading(true);
      const Obj = {
        CommentId: responds?.id,
        CourseId: responds?.courseId,
  ...
  if (isLoadingReplies) return <Loading />;
  ...
  return (
    <>
      {responds?.map((respond) => (
        <div
          key={respond.id}
          className="flex flex-col lg:flex-row justify-center items-center py-2 gap-x-10"
        >
          <div className="bg-gray-300/20 shadow-md dark:bg-gray-700 rounded-lg w-full flex flex-col justify-center items-start px-4 py-2">
            ...
            <div className="flex justify-start items-center gap-x-5 py-5 border-b-2 border-gray-400/50 dark:border-gray-400 rounded-xl w-full">
              ...
              <h4 className="text-gray-600 dark:text-gray-200">{`عنوان : ${responds?.title}`}</h4>
            </div>
                ...
                <>
                  {showReplies ? (
                    <>
                      ...
                      >
                        {respond.replyCount > 1
                          ? "پنهان کردن پاسخ‌ها"
                          : respond.replyCount === 1 && "پنهان کردن پاسخ‌"}
                    ...
                    >
                      {respond.replyCount > 1
                        ...
                        : respond.replyCount === 1 && "نمایش  پاسخ‌"}
